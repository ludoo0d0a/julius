 ...
 * official national list prices from Circle K Sweden via [SwedenFuelPricesClient].
 */
class SwedenOfficialProvider(
    private val swedenClient: SwedenFuelPricesClient,
    ...
    override suspend fun getGasStations(
        ...
        val isSweden = region == ParkingRegion.Sweden || region?.countryCode == "SE"
        ...
        val fuelPrices = if (isSweden) {
            try {
                swedenClient.getFuelPrices().takeIf { it.isNotEmpty() }
            } catch (e: Exception) {
                log.w(e) { "[SwedenOfficialProvider] Failed to fetch prices" }
                null
            }
        } else {
            null
        ...
        return elements.map { el ->
            ...
            Poi(
                ...
                source = if (fuelPrices != null) "OpenStreetMap + Sweden (official list price)" else "OpenStreetMap"
            )
        }
    }
}
